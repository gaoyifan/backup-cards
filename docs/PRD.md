...
*   `{model}`: Camera model detected via `exiftool` (images prioritized over videos; falls back to `UNKNOWN`, spaces replaced with `-`).

> `{model}` requires the `exiftool` binary (configurable via `EXIFTOOL_BIN`); when missing, the placeholder resolves to `UNKNOWN`. Any spaces in the detected model become `-`.
